...
def test_rule_on_jobs_without_needs(rule, no_needs_workflow):
    ...
    assert (
        message == message
    ), "check-run is missing from the following jobs in the workflow: quality"
...
def test_rule_on_dependencies_without_check(rule, dependent_missing_check_workflow):
    ...
    assert (
        message == message
    ), "check-run is missing from the following jobs in the workflow: dependent-job"
...
def test_rule_on_no_branches_workflow(rule, no_branches_workflow):
    ...
    assert result is False
    assert (
        message == "Workflows using pull_request_target can only target the main branch"
    )
